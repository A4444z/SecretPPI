...
# 🚨 注意这里：导入旧版的 scatter_mean 模型
from src.models.glue_cmae_scatter_mean import GlueVAE
...
def main():
    ...
    # 导出 JSON 报告
    json_path = os.path.join(args.output_dir, 'cmae_eval_report.json')
    with open(json_path, 'w') as f:
        json.dump(report, f, indent=4, ensure_ascii=False)

    # 导出特征数据
    feature_path = os.path.join(args.output_dir, 'cmae_test_features.pt')
    torch.save({
        'z1_receptor': all_z1,
        'z2_ligand': all_z2,
        'pdb_ids': all_pdb_ids,
